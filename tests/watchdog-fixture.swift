...
var mockLoaded=true
var mockDisabled="disabled = { com.example-local.dsh-watchdog => true }"
var calls:[[String]]=[]
...
func run(_ executable:String,_ args:[String])throws->(Int32,String){
 ...
 let domain="gui/\(getuid())",service=domain+"/com.example-local.dsh-watchdog"
 let allowed:[[String]]=[["print",service],["print-disabled",domain],["bootout",service],["bootstrap",domain,home.appendingPathComponent("Library/LaunchAgents/com.example-local.dsh-watchdog.plist").path]]
...
@main struct Main{
 static func main()throws{
  let plist=home.appendingPathComponent("Library/LaunchAgents/com.example-local.dsh-watchdog.plist")
  try fm.createDirectory(at:plist.deletingLastPathComponent(),withIntermediateDirectories:true)
  let original=try PropertyListSerialization.data(fromPropertyList:["Label":"com.example-local.dsh-watchdog","StartInterval":20],format:.xml,options:0)
