...
test('cold full installer encrypts, starts, validates, migrates watchdog, and rolls back code preserving new chats',{timeout:180000,skip:privateHelpers?false:'watchdog helpers from the private tree are not present'},async()=>{
 ...
 try{
  ...
  const plist=path.join(home,'Library/LaunchAgents/com.example-local.dsh-watchdog.plist');
  put(plist,{Label:'com.example-local.dsh-watchdog',ProgramArguments:['/synthetic/python','/synthetic/watchdog.py'],StartInterval:20,EnvironmentVariables:{DSH_BRIDGE_RESTART:'/synthetic/old/restart-nodo.sh'}});command('/usr/bin/plutil',['-convert','xml1',plist]);put(path.join(home,'mock-watchdog.json'),{loaded:true});
  const factoplist=path.join(home,'Library/LaunchAgents/com.example-local.vk-factoscope.plist'),calendar=[{Hour:(new Date().getHours()+12)%24,Minute:0}];
  put(factoplist,{Label:'com.example-local.vk-factoscope',ProgramArguments:['/bin/bash','/synthetic/old/vk-factoscope.sh'],RunAtLoad:false,StartCalendarInterval:calendar});command('/usr/bin/plutil',['-convert','xml1',factoplist]);put(path.join(home,'mock-factoscope.json'),{loaded:true});
  ...
  const allCalls=fs.readFileSync(path.join(home,'mock-watchdog-calls.jsonl'),'utf8').trim().split('\n').map(JSON.parse);for(const label of ['com.example-local.dsh-watchdog','com.example-local.vk-factoscope']){assert(allCalls.some(x=>x[0]==='bootout'&&x[1].endsWith('/'+label)));assert(allCalls.some(x=>x[0]==='bootstrap'&&x[2].endsWith('/'+label+'.plist')));}assert(allCalls.every(x=>!['enable','disable'].includes(x[0])));
 }finally{
