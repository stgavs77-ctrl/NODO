...
@main struct Main {
 static func main()throws{
  ...
  for file in excluded{try put(file,Data("SYNTHETIC_EXCLUDED".utf8))}
  let scope=try nodoBackupScope(data:data,userHome:home)
  try checkScope(scope.contains(data),"selected profile absent")
  try checkScope(scope.count==1+nodoBridgeBackupFiles.count+3,"scope cardinality")
  ...
  for file in excluded{let p=home.appendingPathComponent(file).path;try checkScope(!scope.contains{p==$0.path||p.hasPrefix($0.path+"/")},"excluded file selected")}
  ...
  timer.setEventHandler{for file in excluded{try? put(file,Data(("SYNTHETIC_UPDATE_"+String(writes)).utf8))};writes+=1}
  timer.resume()
  ...
  for file in excluded{try checkScope(!fm.fileExists(atPath:restored.appendingPathComponent(file).path),"excluded live file restored")}
