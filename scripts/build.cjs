...
for(const f of ['main.cjs','preload.cjs','package.json','profile.patch.yml','codex-config.toml','dsh-plugin.mjs','lib','services','extension','ui','assets','patches','config','runtime'])run('/bin/cp',['-cR',path.join(root,f),path.join(project,f)]);
