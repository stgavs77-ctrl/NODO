...
// Independent example-local data is no longer part of transactional NODO backup.
...
// The updater end-to-end tests run a released build against their own profile;
// NODO_DATA is inert in a normal launch.
const data=process.env.NODO_DATA&&path.isAbsolute(process.env.NODO_DATA)?process.env.NODO_DATA:path.join(os.homedir(),'Library/Application Support/NODO');
...
async function inspect(){const other=externalBlockers();if(other.length)throw Error('Shared Telegram state still has independent writers (not authorized to stop): '+other.join(', '));let live;try{live=await endpoint('lifecycle.status');}catch{throw Error('OLD_RUNTIME_QUIESCE_UNSUPPORTED: no verified pause/drain protocol. No shutdown or file replacement performed.');}if(live.protocol!==1)throw Error('Unsupported shutdown protocol '+String(live.protocol));
 if(live.activeTurns!==0||live.tasks!==0){
  const busy=(live.services||[]).filter(v=>Number(v.inflight)>0).map(v=>v.name+':'+v.inflight);
  throw Error('Active turns/Tasks: turns='+live.activeTurns+' tasks='+live.tasks+(busy.length?' inflight='+busy.join(','):''));
 }
 return live;}
