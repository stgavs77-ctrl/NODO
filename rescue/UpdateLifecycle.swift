...
// Exact NODO infrastructure only; no independent example-local jobs.
final class WatchdogTransaction {
 ...
 init(_ journal:URL,label:String="com.example-local.dsh-watchdog"){self.journal=journal;self.label=label}
 ...
 func capture() throws {
  guard ["com.example-local.dsh-watchdog","com.example-local.vk-factoscope"].contains(label) else{try fail("Launchd label outside NODO scope")}
 ...
 func suspend() throws {
  ...
  if label=="com.example-local.vk-factoscope" {
 ...
 func migrateLaunchTarget() throws {
  ...
  if label=="com.example-local.dsh-watchdog" {
