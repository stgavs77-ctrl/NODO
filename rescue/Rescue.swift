...
func isOwnedUpdaterPID(_ pid:Int32)->Bool {pid == getpid() || ownedCleanupProcesses.contains(where:{$0.isRunning && $0.processIdentifier == pid})}
func processes(_ markers:[String]) throws -> [String] { let (status,out)=try run("/bin/ps",["-axo","pid=,command="]);guard status == 0 else { try fail("Cannot verify processes") };return out.split(separator:"\n").map(String.init).filter{ line in let pid=Int32(line.trimmingCharacters(in:.whitespaces).split(separator:" ").first ?? "") ?? 0;return !isOwnedUpdaterPID(pid) && markers.contains(where:{!$0.isEmpty && (line.contains($0+"/") || line.hasSuffix($0) || line.contains($0+" --"))}) } }
let defaultData = home.appendingPathComponent("Library/Application Support/NODO")
let defaultApp = home.appendingPathComponent("Applications/NODO.app")
...
func ready(_ app:URL,_ data:URL,safe:Bool=false) throws {
 ...
 let started=Date();guard let info=NSDictionary(contentsOf:app.appendingPathComponent("Contents/Info.plist")),let executable=info["CFBundleExecutable"] as? String,!executable.contains("/") else{try fail("Invalid application executable")};let p=Process();p.executableURL=app.appendingPathComponent("Contents/MacOS/\(executable)");var env=ProcessInfo.processInfo.environment;env.removeValue(forKey:"NODO_DEV");env.removeValue(forKey:"NODO_DATA");env.removeValue(forKey:"NODO_SAFE_MODE");if safe{env["NODO_SAFE_MODE"]="1"};p.environment=env;p.standardOutput=FileHandle.nullDevice;p.standardError=FileHandle.nullDevice;try p.run()
...
func install(_ folder:URL) throws {
 ...
 if test && !(target.hasPrefix(fm.temporaryDirectory.path) || target.hasPrefix("/private/tmp/nodo-rescue-test-")) { try fail("Synthetic install requires temporary target") }
 ...
 let scheduled=WatchdogTransaction(transaction.appendingPathComponent("factoscope/watchdog.json"),label:"com.nodo-optional.vk-factoscope")
 ...
 if !test { let rescue=folder.appendingPathComponent("NODO Rescue.app");if fm.fileExists(atPath:rescue.path){let dest=home.appendingPathComponent("Applications/NODO Rescue.app");if fm.fileExists(atPath:dest.path){try fm.moveItem(at:dest,to:backup.deletingLastPathComponent().appendingPathComponent("NODO Rescue.app"))};try fm.copyItem(at:rescue,to:dest)};do {try ready(targetURL,data);var report:[String:Any]?,healthError="No health response";for _ in 0..<30{do{report=try lifecycleCommand(targetURL,"health");break}catch{healthError=error.localizedDescription};Thread.sleep(forTimeInterval:1)};guard let after=report?["snapshot"] as? [String:Any] else{try fail("Post-start services not healthy: \(healthError). Update NOT accepted; use Repair/Rollback.")};try assertPreserved(before,after);if scheduledCaptured {try scheduled.migrateLaunchTarget();try scheduled.commitLaunchTarget()};try watchdog.migrateLaunchTarget();try watchdog.commitLaunchTarget();try watchdog.restore();if scheduledCaptured {try scheduled.restore()};try write(["phase":"verified","counts":after["counts"] ?? [:]],transaction.appendingPathComponent("result.json")); } catch {let reason=error.localizedDescription;try recoverFailedStartup(targetURL,data,base);try fail("Update NOT accepted: \(reason). Previous code restored and started; user data retained.")} }
 completed=true
}
func rollback(_ base:URL) throws { let cfg=try json(base.appendingPathComponent("last-update.json"));guard let target=cfg["target"] as? String,let backup=cfg["backup"] as? String,let data=cfg["dataPath"] as? String else {try fail("No previous update")};let test=cfg["testOnly"] as? Bool == true;if !test && (target != defaultApp.path || data != defaultData.path){try fail("Unexpected rollback target")};if test && !(target.hasPrefix(fm.temporaryDirectory.path) || target.hasPrefix("/private/tmp/nodo-rescue-test-")){try fail("Invalid test rollback target")};if !(try processes([target,data])).isEmpty {try fail("Close NODO and dependent writers before rollback")};guard backup.hasPrefix(base.appendingPathComponent("backups").path+"/"),backup.hasSuffix("/NODO.app"),!backup.split(separator:"/").contains("..") else{try fail("Invalid rollback path")};let b=URL(fileURLWithPath:backup);guard fm.fileExists(atPath:b.path) else{try fail("Backup missing")};if !test {try manifest(b)};let t=URL(fileURLWithPath:target),stage=t.deletingLastPathComponent().appendingPathComponent(".NODO-rollback-\(UUID().uuidString).app");try fm.copyItem(at:b,to:stage);let displaced=b.deletingLastPathComponent().appendingPathComponent("failed-\(UUID().uuidString).app");if fm.fileExists(atPath:t.path){try fm.moveItem(at:t,to:displaced)};do{try fm.moveItem(at:stage,to:t)}catch{if fm.fileExists(atPath:displaced.path){try? fm.moveItem(at:displaced,to:t)};throw error};var done=cfg;done["phase"]="rolled-back";try write(done,base.appendingPathComponent("last-update.json")) }
...
final class Delegate:NSObject,NSApplicationDelegate {
 ...
 func applicationDidFinishLaunching(_ notification:Notification){window=NSWindow(contentRect:NSRect(x:0,y:0,width:460,height:420),styleMask:[.titled,.closable,.miniaturizable],backing:.buffered,defer:false);window.title="NODO Rescue";window.center();let stack=NSStackView();stack.orientation = .vertical;stack.spacing=12;stack.edgeInsets=NSEdgeInsets(top:24,left:28,bottom:24,right:28);stack.frame=window.contentView!.bounds;stack.autoresizingMask=[.width,.height];window.contentView!.addSubview(stack);label=NSTextField(wrappingLabelWithString:"Независимое восстановление NODO. История остаётся на месте.");stack.addArrangedSubview(label);for (title,tag) in [("Запустить NODO",0),("Безопасный режим",1),("Диагностика и восстановление",2),("Откатить последнее обновление",3),("Открыть журналы",4),("Создать диагностический пакет",5),("Подготовить копию для внешнего Codex",6)] {let b=NSButton(title:title,target:self,action:#selector(action(_:)));b.tag=tag;b.bezelStyle = .rounded;stack.addArrangedSubview(b)};window.makeKeyAndOrderFront(nil);NSApp.activate(ignoringOtherApps:true)}
