...
function article(label,value){if(!value)return;const row=document.createElement('article'),who=document.createElement('small'),body=document.createElement('div');who.textContent=label;body.textContent=value;row.append(who,body);$('messages').append(row);}
async function poll(){if(!ready||!selected||polling||document.hidden)return;polling=true;const id=selected;try{const s=await call('session',{sessionId:id});if(id!==selected)return;const pane=$('messages'),atBottom=pane.scrollHeight-pane.scrollTop-pane.clientHeight<80;pane.replaceChildren();for(const r of s.records||[]){const e=r.event;if(e?.type==='user/message')article('You',text(e.data));if(e?.type==='assistant/message')article('NODO',text(e.data));}article('NODO · streaming',text({stream:s.assistantStream?.activeAttempt?.stream}));if(atBottom)pane.scrollTop=pane.scrollHeight;await showInteractions();}finally{polling=false;}}
function connect(){
...
$('workspaces').onchange=()=>{sessionOptions();poll().catch(showError);};$('sessions').onchange=()=>{selected=$('sessions').value;localStorage.setItem('nodo.session',selected);poll().catch(showError);};
