...
class Checkpoints{
 constructor(data){this.root=path.join(data,'workspace');this.dir=path.join(data,'checkpoints');fs.mkdirSync(this.root,{recursive:true});fs.mkdirSync(this.dir,{recursive:true});}
 ...
 list(){return fs.readdirSync(this.dir).filter(n=>n.endsWith('.json')).map(n=>read(path.join(this.dir,n))).map(({id,at,task,files})=>({id,at,task,files:files.map(f=>f.path)})).sort((a,b)=>b.at.localeCompare(a.at));}
