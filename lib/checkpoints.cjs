...
class Checkpoints{
 ...
 list(){return fs.readdirSync(this.dir).filter(n=>/^[a-f0-9-]{36}\.json$/.test(n)).map(n=>read(path.join(this.dir,n))).map(({id,at,task,files})=>({id,at,task,files:files.map(f=>f.path)})).sort((a,b)=>b.at.localeCompare(a.at));}
