...
const METHODS=new Set(['workspaces','sessions','session','newSession','send','stop','interactions','answer','project','media']);
...
function createRemoteAPI({dsh,paused=()=>false}){
 return async function call(method,p={}){
  ...
  if(method==='media'){fields(p,['sessionId','path']);id(p.sessionId);if(typeof p.path!=='string'||p.path.length>4096||!p.path.startsWith('/'))throw Error('Invalid media path');return dsh('remote.media',p);}
  if(method==='project'){fields(p,['sessionId','view']);id(p.sessionId);if(!['brain','rules','context','mission'].includes(p.view))throw Error('Read-only project view required');return dsh('remote.project',p);}
