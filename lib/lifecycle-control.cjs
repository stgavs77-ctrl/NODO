...
function lifecycleControl(ctx,isolated){
 ...
 return {gate,status,get paused(){return paused;},
  ...
  async pause(){paused=true;try{const missing=required.filter(n=>!registry.has(n));if(missing.length)throw Error('Lifecycle services missing: '+missing.join(', '));await Promise.all(required.map(n=>registry.get(n).pause()));const s=status();if(s.activeTurns!==0||s.services.some(x=>!x.drained))throw Error('Background work has not drained; code update forbidden');return s;}catch(e){
   // A failed pause must not leave prompts blocked: reopen admission and resume what did pause.
   paused=false;for(const n of required){try{registry.get(n)?.resume();}catch{}}throw e;}},
