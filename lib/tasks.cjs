...
class Tasks{
 constructor(app){this.app=app;this.file=path.join(app.data,'tasks.json');this.items=read(this.file,[]);for(const t of this.items)if(['Running','Waiting'].includes(t.status)){t.status='Failed';t.error='RC closed before task completion; start a new turn to continue';}this.app.codex.on('event',m=>this.codexEvent(m).catch(e=>{this.app.codex.lastError=e.message;}));this.app.codex.on('offline',e=>{for(const t of this.items)if(t.agent==='Codex'&&['Running','Waiting'].includes(t.status)){t.status='Failed';t.error=e;}this.changed();});this.changed();}
 changed(){save(this.file,this.items.map(({timer,...t})=>t));this.app.changed();}
 ...
 if(!t)return;
 if(m.method==='item/agentMessage/delta'){t.output+=p.delta||'';this.changed();}
 ...
 async cancel(id){const t=this.get(id);if(t.agent==='Codex'&&t.threadId&&t.turnId)await this.app.codex.request('turn/interrupt',{threadId:t.threadId,turnId:t.turnId});else if(t.sessionId)await this.app.dsh('cancel',{sessionId:t.sessionId});t.status='Paused';t.lastAction='Stopped by user';if(t.timer)clearTimeout(t.timer);delete t.timer;this.changed();}
}
