...
class Supervisor {
  ...
  enforceStop(sessionId) {
    ...
    // A hung tool must not keep the turn alive forever: after a bounded number of
    // extra grace periods the turn is cancelled even with an operation in flight.
    state.graceWaits = (state.graceWaits || 0) + 1;
    if (state.inFlight > 0 && state.graceWaits <= MAX_GRACE_WAITS) {
      state.stopTimer = setTimeout(() => this.enforceStop(sessionId), this.thresholds.graceMs);
      if (state.stopTimer.unref) state.stopTimer.unref();
      return;
    }
  ...
  stopTurn(state, reason) {
    try {
      // cancel() is async in the harness; a rejection must be logged, not left unhandled.
      if (typeof this.cancel === 'function') Promise.resolve(this.cancel(state.id, reason)).catch(error => this.warn('cancel failed: ' + (error && error.message)));
    } catch (error) {
