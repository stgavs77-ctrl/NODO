...
const QUESTION_RE = /(^\s*(?:что|кто|где|когда|почему|зачем|как|какой|какая|какие|сколько|можно|нужно ли|стоит ли|есть ли|будет ли|объясни|расскажи|поясни|подскажи|сравни)\b|^\s*(?:what|who|where|when|why|how|which|can|could|should|would|is|are|do|does|did|explain|tell me|compare)\b|\?\s*$)/i;
...
function classifyUserMessage(text) {
  ...
  if (/\?/.test(value) || QUESTION_RE.test(value)) return 'question';
