...
function coldPreflight(root,bridge){
 ...
 if(!Array.isArray(rows)||rows.some(t=>!['done','failed','cancelled','canceled','paused'].includes(String(t.status).toLowerCase())))throw Error('Finish saved Tasks before updating');
 let sessions=0;
 ...
 // A profile without the Telegram bridge has no shared queue to verify. That is
 // the normal case for an isolated profile, not an unverifiable state.
