...
class UsageLedger{
 ...
 recordDeepSeek(sessionId,events){let changed=false;for(const e of events||[]){if(e.type!=='assistant/message')continue;const u=deepseekUsage(e.data?.usage);if(u&&Number.isSafeInteger(e.seq)){const key='deepseek:'+sessionId+':'+e.seq;if(!this.rows[key]){this.rows[key]={provider:'DeepSeek',sessionId,...u};changed=true;}}}if(changed)save(this.file,this.rows);return this.forSession(sessionId);}
