...
<p align="center">
  ...
  <img alt="Version 1.3.0" src="https://img.shields.io/badge/version-1.3.0-black?style=flat-square">
...
ready-to-run binary from the latest GitHub Release (current: **NODO 1.3.0**, `NODO-1.3.0.zip`).
...
> **First launch:** NODO 1.3.0 is ad-hoc signed, not notarized with an Apple Developer ID yet. macOS
...
- **Measured context and cost economy.** NODO's context layer decides what actually goes into a
  request and shows what it costs. On a real 11-hour working period it cut model spend by about
  **43%** against the previous context settings - see
...
<p align="center">
  <strong>2,139 requests</strong> &nbsp;·&nbsp; <strong>11 hours of active use</strong> &nbsp;·&nbsp;
  <strong>43% lower model spend</strong> &nbsp;·&nbsp; <strong>44% lower average context</strong>
</p>

One real working period, not a synthetic benchmark: **13 Sep 2026, 11 hours of active NODO use,
2,139 model requests.**
...
<sub>
**Method and scope.** These numbers come from one measured period of real work on one machine, with
NODO's own cache and cost meter as the source. They report what happened in that period; they are not
a guarantee, an average or a promise for every user. The saving depends on project size, context mode,
how much of a workload is cache-friendly and which provider and model you use.

**Provider off-peak pricing is separate.** DeepSeek off-peak pricing was in effect on that day and
reduced provider prices by roughly another 50%. That is an external pricing effect and is **not**
included in the 43% above. Combined with the off-peak discount, the effective total spend for that
workload was roughly **71% lower** than the same workload at peak pricing - a combined effect of NODO
optimization **plus** provider off-peak pricing, not NODO's own saving.
</sub>
...
1. Download `NODO-1.3.0.zip` from [Releases](https://github.com/stgavs77-ctrl/NODO/releases/latest),
