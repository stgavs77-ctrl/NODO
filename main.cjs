...
class Workstation{
 ...
 async start(){
  app.dock?.setIcon(path.join(sourceRoot,'assets/NODO.iconset/icon_512x512@2x.png'));
 ...
 async recordHealthyStartup(){if(config.isolated)return;for(let i=0;i<30&&!this.stopped;i++){try{const s=await this.dsh('lifecycle.status');if(s.services.every(v=>!v.missing&&!v.paused)){save(path.join(config.rescue,'last-known-good.json'),{app:path.resolve(sourceRoot,'../../..'),verifiedAt:new Date().toISOString(),version:config.version,verification:'DSH and required services ready; Codex '+(this.codex.ready?'ready':'unavailable (optional)')});return;}}catch{}await new Promise(r=>setTimeout(r,1000));}status('degraded',{error:'SERVICE_HEALTH_UNCONFIRMED'});}
 ...
 async stopGracefully(reason='user'){if(this.stopPromise)return this.stopPromise;this.stopPromise=(async()=>{this.shutdownReason=reason;this.stopping=true;await this.features?.stop();if(this.dshChild?.exitCode===null)await this.dispatch('lifecycle.pause',{},false);if(this.saveTimer)clearTimeout(this.saveTimer);save(this.stateFile,this.state);if(this.tasks?.saveTimer)this.tasks.changed();this.balance.stop();
  // Mark Codex as intentionally stopped so its exit does not trigger an automatic restart.
  this.codex.stopped=true;clearTimeout(this.codex.restartTimer);const children=[this.dshChild,this.codex.child].filter(c=>c&&c.exitCode===null);await Promise.all(children.map(child=>new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(Error('A child has not exited; no force kill was used')),30000);child.once('exit',()=>{clearTimeout(timer);resolve();});child.kill('SIGTERM');})));this.browser?.dispose();this.server?.close();this.testServer?.close();try{fs.rmSync(this.temp,{recursive:true,force:true});}catch{}this.stopped=true;status('stopped',{reason});})().catch(e=>{this.stopPromise=null;throw e;});return this.stopPromise;}
}
